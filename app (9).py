...
fuel_type_input = st.sidebar.selectbox('Jenis Bahan Bakar', list(fuel.keys()))
seller_type_input = st.sidebar.selectbox('Tipe Penjual', list(seller_type.keys()))
transmission_type_input = st.sidebar.selectbox('Transmisi', list(transmission.keys()))
owner_status_input = st.sidebar.selectbox('Jumlah Pemilik Sebelumnya', list(owner.keys()))
...
if st.sidebar.button('Prediksi Harga Mobil'):
    ...
    if car_name_input not in valid_car_names:
        ...
        # Konversi kategori
        age = 2025 - year_input
        fuel_encoded = fuel[fuel_type_input]
        seller_type_encoded = seller_type[seller_type_input]
        transmission_encoded = transmission[transmission_type_input]
        owner_encoded = owner[owner_status_input]
